...
L = np.array([20., 40., 20., 8.])
k = np.array([1.241, 1.0758, 0.6, 1500., 1500., 1500., 1500., 1500.])
# LC_val = [tForceStart, tForceEnd, amp, freqIn, phi0, c1, c2] for LC_case 1
LC = np.array([[int((N_global+1)/2),1,2]])
amp = 0.1
freqIn = 20.0
phi0 = 0.0
t1 = 2.0
...
DoF = 6
noState = 2*DoF
m = np.array([2.e-6, 2.e-6, 1.e-6, 1.e-6, 1.e-6, 1.e-6]) * np.ones((N_global,DoF))
zeta = 0.0
freq0 = 26.281
b = 2*zeta*2*np.pi*freq0*m
x = np.zeros((N_global,noState))
#x[0, 1*noState-11] = 100.0
